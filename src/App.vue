  ...
  <div>
    <div>
          ...
          <transition-group name="flip-list">
            <template v-for="(item, index) in items" v-bind:key="item.id">
              <div
                ...
                style="width: fit-content; display: inline-block;border-width:1px"
              >
      ...
      <div class="row" style="margin-bottom:4px;">
        <div class="col-xs-8">
          <button class="col-xs-8 btn btn-success" @click="calculate">
            开始合成
          </button>
...
//@debug-only
console.assert(
  bucket_sort_list_toint64([0x16, 0x2, 0x16, 0x16, 0x16, 0x16, 0x16, 0x16]) ==
    BigInt("0x1616161616161602")
);
...
// function search_dfs(holder, arr, callback, arr_length, next_holder = 0, next_arr = 0){
//     if(next_holder + 1 == holder.length){
//         //we have no choice
//         for(let i=next_arr;i<arr_length;i++){
//             arr[i] = holder[next_holder]
//         }
//         callback(arr)
//         return
//     }

//     for(let i=0;i<=arr_length - next_arr;i++){
//         for(let j=0;j<i;j++){
//             //fill the array
//             arr[next_arr+j]=holder[next_holder]
//         }
//         if(next_arr + i == arr_length){
//             //no more array
//             callback(arr)
//             continue
//         }
//         search_dfs(holder, arr, callback, arr_length, next_holder+1, next_arr+i)
//     }
// }
    ...
    calculate() {
      ...
      search_dfs(
        ...
        (arr) => {
          ...
          let result = get_result(arr, seed, craftable_arr);
      ...
      for (let i = 1; i < this.results.length; i++) {
        if (items[i]) {
          this.results[i].valid = true;
          this.results[i].crafted = true;
          this.results[i].craftable = true;
          this.results[i].items = items[i];
          craftable_count++
          crafted_count++
        }else{
          this.results[i].crafted = false;
          this.results[i].valid = craftable_arr[i];
          this.results[i].craftable = craftable_arr[i];
          this.results[i].items = [0,0,0,0,0,0,0,0]
          if(craftable_arr[i])
            craftable_count++
        }
      }
      this.craftable_count = craftable_count
      this.crafted_count = crafted_count
    },
  },
};
</script>
